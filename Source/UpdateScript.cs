        ...
        private void Update()
        {
            if (PhotonNetwork.IsMasterClient)
            {
                for (int i = awaitingCamera.Count - 1; i >= 0; i--)
                {
                    ...
                    POVPlugin.Logger.LogInfo("Attempting to summon a camera for " + player.NickName);
            ...
            foreach (VideoCamera cam in UnityEngine.Object.FindObjectsOfType<VideoCamera>())
            {
                ...
                foreach (ItemDataEntry entry in entries)
                {
                    ...
                    if (!matched)
                    {
                        POVPlugin.Logger.LogInfo("Could not find a matching player. Assuming disconnect.");
